...
| Test entry point | `tests/main_test_runner.gd` |
...
- Design, roadmap, and architecture notes live in `src/docs/`.

## Repository Layout

```text
project.godot        Godot project configuration
src/                 Canonical runtime game code
...
tests/               Canonical automated test scripts
res/                 Legacy duplicate tree; ignore in favor of top-level src/ and tests/
```

The top-level `src/` and `tests/` folders are the authoritative locations for runtime code and tests.

## Quick Start

1. Open the repository folder in Godot 4.7.
2. Open `project.godot` as the project root.
...
- Keep runtime scripts in `src/` and tests in `tests/`.
