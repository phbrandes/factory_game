...
func _test_build_mode_toggle() -> void:
	...
	controller.setup(grid, replay, dummy_scheduler, null, menu)
	
	assert(controller.current_build_item_id == "", "Should not start in build mode.")
	
	# Simulate selecting an item from the menu
	controller._on_build_item_selected("belt_t1")
	assert(controller.current_build_item_id == "belt_t1", "Failed to enter build mode.")
	
	# Simulate right click (Cancel)
	var cancel_event = InputEventAction.new()
	cancel_event.action = "ui_cancel"
	cancel_event.pressed = true
	controller._unhandled_input(cancel_event)
	
	assert(controller.current_build_item_id == "", "Failed to exit build mode via cancel action.")